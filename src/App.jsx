...
function AdminPanel({users,setUsers,series,setSeries,siteConfig,setSiteConfig,packs,setPacks,avantages,setAvantages,testimonials,setTestimonials,onLogout}) {
  ...
  return (
    <div style={{minHeight:"100vh",display:"flex",fontFamily:"'DM Sans',sans-serif"}}>
      ...
      {/* SIDEBAR */}
      <aside style={{width:220,background:"#060e1a",display:"flex",flexDirection:"column",borderRight:"1px solid rgba(255,255,255,0.05)",flexShrink:0,position:"sticky",top:0,height:"100vh"}}>
        <div style={{padding:"16px 16px 12px",borderBottom:"1px solid rgba(255,255,255,0.06)"}}>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <div style={{width:26,height:26,borderRadius:7,background:G,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontWeight:900,fontSize:10,fontFamily:"'Playfair Display',serif"}}>LP</div>
            <div>
              <div style={{fontFamily:"'Playfair Display',serif",fontSize:13,fontWeight:700,color:"#fff"}}>LaunchPad</div>
        ...
        <div style={{padding:"12px 14px",borderTop:"1px solid rgba(255,255,255,0.06)"}}>
          <div style={{fontSize:10,color:"rgba(255,255,255,0.3)",marginBottom:6}}>admin@launchpad.ca</div>
        ...
        {tab==="dashboard"&&(
          <div style={{padding:"32px"}}>
            <div className="fu" style={{marginBottom:24}}>
              <h1 style={{fontFamily:"'Playfair Display',serif",fontSize:26,fontWeight:900,color:DARK,marginBottom:3}}>Tableau de bord</h1>
              <p style={{fontSize:13,color:GRAY}}>LaunchPad TCF Canada — Aperçu de la plateforme</p>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:24}}>
              {[
                ...
                {icon:"📋",val:series.reduce((a,s)=>a+(s.questions?.length||0),0),label:"Questions totales",sub:"CE + CO",color:"#7c3aed"},
              ].map((s,i)=>(
                <div key={i} className="card fu" style={{animationDelay:`${i*.06}s`,padding:18,cursor:"default"}}>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}><span style={{fontSize:20}}>{s.icon}</span><div style={{width:7,height:7,borderRadius:"50%",background:s.color,marginTop:3}}/></div>
                  <div style={{fontFamily:"'Playfair Display',serif",fontSize:26,fontWeight:900,color:s.color,marginBottom:2}}>{s.val}</div>
                  <div style={{fontSize:12,fontWeight:600,color:DARK,marginBottom:1}}>{s.label}</div>
                  <div style={{fontSize:11,color:GRAY}}>{s.sub}</div>
                </div>
              ))}
            </div>
            {/* Quick actions */}
            <div className="card" style={{padding:22,marginBottom:16}}>
              <h3 style={{fontFamily:"'Playfair Display',serif",fontSize:15,fontWeight:700,color:DARK,marginBottom:14}}>Actions rapides</h3>
              <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
                <button className="btn btn-p btn-sm" onClick={()=>{setTab("series");setTimeout(()=>setModal({type:"create",serieType:"CE"}),100);}}>+ Nouvelle série CE</button>
                <button className="btn btn-p btn-sm" onClick={()=>{setTab("series");setTimeout(()=>setModal({type:"create",serieType:"CO"}),100);}}>+ Nouvelle série CO</button>
                <button className="btn btn-o btn-sm" onClick={()=>setTab("users")}>Gérer les utilisateurs</button>
              </div>
            </div>
            {/* Progression bars */}
            <div className="card" style={{padding:22}}>
              <h3 style={{fontFamily:"'Playfair Display',serif",fontSize:15,fontWeight:700,color:DARK,marginBottom:14}}>Progression des séries</h3>
                ...
                return(
                  <div key={u.id} style={{background:"#fff",border:`1.5px solid ${u.status==="suspendu"?"rgba(220,38,38,0.2)":BORDER}`,borderRadius:11,padding:"13px 16px",display:"flex",alignItems:"center",gap:12}}>
                    ...
                    <div style={{display:"flex",gap:8,alignItems:"center",flexShrink:0}}>
                      <select value={u.plan} onChange={e=>updateUser(u.id,{plan:e.target.value})} style={{padding:"4px 8px",border:`1.5px solid ${BORDER}`,borderRadius:7,fontSize:11,fontFamily:"'DM Sans',sans-serif",outline:"none",background:"#fff"}}>
                        <option value="free">Free</option>
                        <option value="premium">Premium ⭐</option>
                      </select>
                      <select value={u.status} onChange={e=>updateUser(u.id,{status:e.target.value})} style={{padding:"4px 8px",border:`1.5px solid ${BORDER}`,borderRadius:7,fontSize:11,fontFamily:"'DM Sans',sans-serif",outline:"none",background:"#fff"}}>
                        <option value="actif">✅ Actif</option>
                        <option value="suspendu">🚫 Suspendu</option>
                      </select>
                      <button onClick={()=>deleteUser(u.id)} style={{padding:"5px 9px",border:"1.5px solid rgba(220,38,38,0.3)",borderRadius:7,background:"rgba(220,38,38,0.05)",fontSize:11,cursor:"pointer",color:"#dc2626",fontFamily:"'DM Sans',sans-serif"}}>🗑</button>
                    </div>
                  </div>
                );
              })}
