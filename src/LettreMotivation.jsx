...
export default function LettreMotivation({ user, isPremium }) {
  ...
  const generate = async () => {
    ...
    try {
      const data = await apiPost("/api/ai/lettre", { poste, entreprise, ton, langue, user });
      setLettre(data?.lettre || data?.error || "Erreur inconnue.");
    } catch(e) {
      setLettre("Erreur lors de la génération. Vérifiez votre connexion.");
    }
    setLoading(false);
  };
    ...
    <div style={{maxWidth:760,margin:"0 auto",padding:"32px 16px"}}>
      ...
      <div className="card" style={{padding:24,marginBottom:20}}>
        ...
        <div style={{marginBottom:14}}>
          <label style={{display:"block",fontSize:11,fontWeight:700,color:DARK,marginBottom:8,textTransform:"uppercase",letterSpacing:.4}}>Langue</label>
          <div style={{display:"flex",gap:10}}>
            {[["fr","🇫🇷 Français"],["en","🇨🇦 English"]].map(([v,l])=>(
              <button key={v} onClick={()=>setLangue(v)} style={{
                flex:1,padding:"10px",border:`2px solid ${langue===v?BLUE:BORDER}`,borderRadius:10,
        ...
        <div style={{marginBottom:20}}>
          <label style={{display:"block",fontSize:11,fontWeight:700,color:DARK,marginBottom:8,textTransform:"uppercase",letterSpacing:.4}}>Ton de la lettre</label>
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10}}>
            {Object.entries(TON).map(([v,{label,desc,icon}])=>(
              <button key={v} onClick={()=>setTon(v)} style={{
                padding:"12px 10px",border:`2px solid ${ton===v?BLUE:BORDER}`,borderRadius:12,
                background:ton===v?"rgba(26,58,143,0.06)":"#fff",cursor:"pointer",
                textAlign:"center",transition:"all .15s",fontFamily:"'DM Sans',sans-serif"}}>
        ...
        <button onClick={generate} disabled={loading||!poste.trim()} style={{
          width:"100%",padding:"13px",
          background:poste.trim()?G:"#e4e8f0",border:"none",borderRadius:12,
          color:poste.trim()?"#fff":GRAY,fontSize:14,fontWeight:700,
          cursor:poste.trim()?"pointer":"not-allowed",
        ...
        <div className="card" style={{padding:24}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
            ...
            <div style={{display:"flex",gap:8}}>
              <button onClick={copy} className="btn btn-o btn-sm">{copied?"✅ Copié":"📋 Copier"}</button>
              <button onClick={print} className="btn btn-p btn-sm">🖨️ Imprimer / PDF</button>
            </div>
          </div>
          ...
          <div style={{marginTop:14,padding:"12px 16px",background:"rgba(26,58,143,0.05)",
            ...
            💡 Relisez et personnalisez avant d'envoyer. Ajoutez des détails spécifiques à votre expérience.
          </div>
        </div>
      )}
    </div>
